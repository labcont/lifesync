...
def lock_text_input(user_id):
    USER_INPUT_LOCK[user_id] = time.time()


def unlock_text_input(user_id):
...
@dp.callback_query(
    F.data.startswith("del_")
    & ~F.data.startswith("del_yes_")
    & ~F.data.startswith("del_affirm_")
    & ~F.data.startswith("del_visual_")
    & (F.data != "del_back")
)
...
def normalize_title(name: str):
    name = re.sub(r"^\[[ABC]\]\s*", "", name).strip()
...
def get_smart_feedback(done, total):
    ...
    if done == 0:
        return random.choice([
            "План есть. Пора начинать ⚡",
            "Ты подготовился — переходи к действиям 🧠"
        ])

    if done == 1:
        return random.choice([
            "Хорошее начало. Продолжай 💪",
            "Ты уже в процессе. Не останавливайся 🔥"
        ])

    if done == total:
        return random.choice([
            "Идеально выполнено 🏆",
            "Все задачи закрыты. Сильный результат 💎"
        ])

    if percent >= 70:
        return random.choice([
            "Почти готово. Дожми ⚡",
            "Финиш близко. Не сбавляй темп 🔥"
        ])

    return random.choice([
        "Держишь ритм. Продолжай 🧠",
        "Хороший темп. Не теряй концентрацию ⚡"
    ]) 
...
async def open_productivity_settings(c: CallbackQuery):
    ...
    try:
        await c.message.edit_text(
            "⚡ <b>Продуктивная система</b>\n"
            "«Выйди из зоны комфорта»\n"
            "Брайан Трейси\n\n"

            "1. Начинай с главного — сначала выполняй самую важную задачу\n"
            "2. Делай сложное первым — «съешь лягушку» утром\n"
            "3. Работай по приоритетам — A / B / C вместо хаоса\n"
            "4. Фокусируйся на одном — не распыляй внимание\n"
            "5. Планируй день заранее — ясность убирает прокрастинацию\n"
            "6. Действуй сразу — не откладывай важное\n"
            "7. Контролируй прогресс — измеряй результат\n"
            "8. Убирай лишнее — всё, что не даёт результата, убирается\n"
            "9. Доводи до конца — незавершённые задачи съедают энергию\n"
            "10. Дисциплина > мотивация — система важнее настроения\n\n"

            "Дополнительно:\n"
            "— 1 главная задача в день даёт 80% результата\n"
            "— Приоритет A = критично, B = важно, C = желательно\n"
            "— Фокус = скорость + качество\n\n"
...
async def habits_menu_handler(message: Message):
    ...
    if get_tips(message.from_user.id):
        text += (
            "\n\n💡Подсказка<blockquote>"
            "Создавай привычки и задачи нажав ➕\n\n"
            "• Создание привычки\n"
            "• 👥Личные (для себя) и общие(для семьи)\n"
            "• 🔁 Цикличная (обновляеться еженедельно) и 🎯 Разовая\n"
            ...
            "• 🗓Настраивай дни и время️\n\n"
            "Затем:\n"
            "— ✔Отмечай выполнение, нажав на название в меню управление и 🔥Набирай стрик \n"
            "— 🔎Смотри весь список за нелелю/месяц/все время в меню прогресс\n"
            "— *Если активна премиум функция «Выйди из зоны комфорта» используй:\n"
            "— 🏆 Главная задача → фокус на одном результате\n"
            "— ⚡ Приоритеты A/B/C → чёткая система выбора\n"
            "— 📅 План дня → контроль выполнения и эффективности\n"
            "</blockquote>"
...
async def delete_affirm_menu(c: CallbackQuery):
    ...
    if not rows:
        await c.answer("Нет аффирмаций", show_alert=True)
        return await affirm_menu(c)
...
async def delete_visual_menu(c: CallbackQuery):
    ...
    if not rows:
        await c.answer("Нет картинок", show_alert=True)
        return await visual_menu(c)
